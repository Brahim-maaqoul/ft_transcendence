...
export class ChatController {
    ...
    @Post('/jounGroup')
    @UseGuards(AuthGuard('jwt'))
    async jounGroup(@Body() Body, @Res() res,@Req() request) {
        try {
        const  userId = request.user.auth_id
        const  {groupId,password,type} = Body
        console.log("groupId",groupId,"password",password,"type",type)
        await this.ChatService.jounGroup(userId,groupId,password,type);
        return res.status(200).json({ message: 'Group Join successfully' });
        } catch (error) {
            console.log("Failed to Join the group'")
        return res.status(500).json({ message: 'Failed to Join the group' });
        }
    }
    @Post('/checkMember')
    @UseGuards(AuthGuard('jwt'))
    async checkMember(@Body() Body, @Res() res,@Req() request) {
        try {
        const  userId = request.user.auth_id
        const  {groupId} = Body
        const isMember = await this.ChatService.checkMember(userId,groupId);
        console.log("isMember",isMember)
        return res.status(200).json({ isMember: isMember});
        } catch (error) {
            return res.status(500).json({ message: 'Failed to create the group' });
        }
    }
    ...
    async addFriendToGroup(@Body() Body, @Res() res,@Req() request) {
        ...
        console.log("userId2",userId2,"groupId",groupId,"userId",userId)
    ...
    async getMemberGroup(@Body() Body, @Res() res,@Req() request) {
        try {
        console.log("Wsal hnaaaaaaaaa ***********")
        const  userId = request.user.auth_id
        const  {groupId} = Body
        const members = await this.ChatService.getMemberGroup(groupId,userId);
        return res.status(200).json(members);
        } catch (error) {
            return res.status(500).json({ message: 'Failed to get Member of the group' });
        }
    }
    @Post('/removeRome')
    @UseGuards(AuthGuard('jwt'))
    async removeRome(@Body() Body, @Res() res,@Req() request) {
        ...
        const  userId = request.user.auth_id
        const  {groupId} = Body
        const result  = await this.ChatService.removeRome(userId,groupId);
