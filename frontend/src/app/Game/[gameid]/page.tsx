...
const initializeGame = (): GameState => ({
  ball: createBall(50, 10, 0.7 * 0.3, 0.7 * 0.3),
...
const updateGame = (game: GameState): GameState => {
  ...
  if (game.ball.x <= 0 - game.ball.width || game.ball.x >= 100) {
    game.ball = createBall(50, 10, 0.7 * speed, 0.7 * speed);
  }

  if (Math.abs(game.ball.x - game.racket1.width) <= 0.1) {
    let diff = game.ball.y - newRacket1Y;
    if (diff >= 0 && diff <= game.racket1.height) game.ball.dx = 0.7 * speed;
  }

  if (
    Math.abs(100 - game.racket2.width - game.ball.width - game.ball.x) <= 0.1
  ) {
    let diff = game.ball.y - newRacket2Y;
    if (diff >= 0 && diff <= game.racket2.height) {
      game.ball.dx = -0.7 * speed;
    }
  }
  if (game.ball.y <= 0) {
    game.ball.dy *= -1;
...
export default function Game2d() {
  ...
  return (
    <div className="h-full relative flex flex-col justify-between xl:justify-center  items-center">
        ...
        <div className="">
          ...
          <div>
            <div className=" absolute left-0 right-0 bottom-0 top-0 m-auto  w-[150px] h-[150px]  rounded-full  bg-white">
              <div className="absolute left-0 right-0 bottom-0 top-0 m-auto  w-[140px] h-[140px]  rounded-full  bg-[#d48686]"></div>
            </div>
            <div className=" absolute left-[50%] right-[50%] w-[2px] sm:w-[5px] bottom-0 top-0 bg-white"></div>
            <div
              style={{
                position: "absolute",
      ...
      <button className=" mx-auto  px-10 py-2 bg-[#d48686] rounded-full m-10 text-white">
        Quit
      </button>
    </div>
  );
